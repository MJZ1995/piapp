...
export function AppShell() {
  ...
  const windowTitle = activeCwdName ? `${activeCwdName} — Yasuo Agent` : "Yasuo Agent";
  ...
  const renderSessionStatsButton = (mobile: boolean) => {
    ...
    if (tokens) {
      ...
      const hitRate = tokens.input + tokens.cacheRead + tokens.cacheWrite > 0
        ? (tokens.cacheRead / (tokens.input + tokens.cacheRead + tokens.cacheWrite)) * 100
        : null;
      if (hitRate !== null && (tokens.cacheRead > 0 || tokens.cacheWrite > 0)) {
  ...
  return (
    ...
    {modelsConfigOpen && <ModelsConfig cwd={activeCwd ?? selectedSession?.cwd ?? newSessionCwd} onClose={() => { setModelsConfigOpen(false); setModelsRefreshKey((k) => k + 1); }} />}
